    ...
    try:
        ...
        if result_config:
            ...
            if full_ == False:
                ...
                intersected_df = intersected_df.head(amount_)
            if null_value == True:
                result_list = list(filter(None, result_list))
                intersected_df = intersected_df.drop_nulls()
        # reply_callback(result_list, request_json, "")
        reply_callback(intersected_df, request_json, "")
    except Exception as e:
    ...
    try:
        ...
        if query_config_:
            ...
            if query_key_[0] not in return_fields_:
                return_fields_.insert(0, query_key_[0])
            result_data = filtered_dataset.select(return_fields_)
        
        logger.info(f"pir result_data: {result_data}")
        return reply_callback(result_data, request_json, "", callback)
        
    except Exception as e:
        print("Exception error", str(e))
        reply_callback(result_data, request_json, str(e))
        logger.error(str(e), exc_info=True)
