...
def reply_callback(data, request_json):
...
def task_psi(request_json):
    ...
    try:
        ...
        for config in relational_config:
            ...
            df1 = g_dataset_map[dataset_list[0]]
            if len(dataset_list) == 0:
                ...
                g_dataset_map[dataset_list[0]] = df1
                if len(dataset_list) > 1:
                    df2 = g_dataset_map[dataset_list[1]]
...
def get_result_file():
    ...
    try:
        ...
        # dataset = g_dataset_map[dataset_code_]
        dataset = g_dataset_map.pop(dataset_code_, None)
        if dataset is None:
